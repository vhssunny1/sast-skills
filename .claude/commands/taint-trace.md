...
### The call graph for a typical web app looks like this:

**Synchronous path (Java/Spring, Flask, FastAPI):**
```
HTTP Request
    ↓
[Entry Point — Controller / View]       ← user data enters here (HTTP params, cookies, headers)
    ↓  calls
...
**Async queue path (Celery / RQ / Dramatiq):**
```
HTTP Request
    ↓
[Entry Point — View / Route handler]    ← user data enters here
    ↓  enqueue(func, tainted_args)
[Message Broker — Redis / RabbitMQ]     ← taint crosses process boundary here (data is serialized)
    ↓  dequeue
[Async Worker — task function]          ← receives tainted_args as function parameters
    ↓
[Dangerous sink in worker]              ← subprocess, exec, DB write, file write
```
For async findings, trace from entry_point → enqueue call, then separately from worker function → sink. The broker hop is implicit — mark it in `taint_path` as `role: "async_queue"`.

**Feature-flag gated path:**
```
HTTP Request
    ↓
[Guard check: if not settings.FEATURE_ENABLED: return/redirect]   ← conditional protection
    ↓  (only when flag is true)
[Dangerous code path]                   ← auth bypass, unsafe handler, etc.
```
For conditional findings, note the guard in `taint_path` and set `conditional_protection: true` on the finding. Conditional protection is NOT effective sanitization — it is a deployment-time configuration. The vulnerability is real; the condition scopes when it is exploitable.
...
- For async queue hops, insert a step with `role: "async_queue"` and `file: "(broker)"` — no source file to cite, but the broker hop must be represented
- For conditional guard steps, insert a step with `role: "conditional_guard"` noting which env var or config flag gates the path
